...
def allowed_file(filename):
    return '.' in filename and \
           filename.rsplit('.', 1)[1] in inventory.config['ALLOWED_EXTENSIONS']
...
@inventory.route('/inventory/form', methods = ['GET', 'POST'])
def submit_form():
	...
	if request.method == 'POST':
			...
			if file and allowed_file(file.filename):
				...
				file.save(os.path.join(inventory.config['UPLOAD_FOLDER'], filename))
...
@inventory.route('/inventory/quick', methods = ['GET', 'POST'])
def quick():
	sku_input = request.args.get('sku', None)
	form = product_submit()
...
@inventory.route('/inventory/product/check', methods = ['GET', 'POST'])
def index():
...
@inventory.route('/inventory/stat')
def stats():
	places = []
	f = []
	places_count = {}
